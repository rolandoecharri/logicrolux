...
def create_soap_request():
    ...
    login = os.getenv('LOGIN', 'ltqt@lsgps.com.ar')
    password = os.getenv('PASSWORD', 'logictracker')
  
    # Fechas: endDate = ahora, startDate = 2 minutos atrás
...
def parse_soap_response(xml_response):
    """Parsea la respuesta SOAP y extrae los datos de posición"""
    positions = []
    try:
        root = ET.fromstring(xml_response)
        # Buscar todos los elementos de posición (ajustar según la estructura real de la respuesta)
...
def execute_soap_request():
    ...
    try:
        ...
        # Parsear respuesta y enviar POSTs
        positions = parse_soap_response(response.text)
        post_logger.info(f"Posiciones encontradas: {len(positions)}\n")
...
<body>
    <div class="container">
        ...
        <div class="section">
            <h2>📤 Resultados POST ({{ post_count }} registros)</h2>
            <div id="postResults">
