...
latest_post_results = []
last_update = ""
device_status = {}  # Diccionario para almacenar el estado de cada dispositivo
...
def create_soap_request():
    ...
    # Fechas: endDate = ahora, startDate = 15 minutos atrás (con timezone UTC)
   ...
   <soapenv:Body>
    ...
    return soap_request, start_date
...
def parse_soap_response(xml_response):
    ...
    try:
        ...
        for item in root.iter():
            if 'unitPlate' in item.tag or any(child.tag.endswith('unitPlate') for child in item):
                ...
                if position and 'unitPlate' in position and 'dateGps' in position:
                    ...
                    try:
                        # Convertir fecha a datetime para comparar (con timezone)
        ...
        # Actualizar device_status con las últimas fechas
        for plate, data in positions_by_plate.items():
            device_status[plate] = {
                'last_report': data['datetime'],
                'last_report_str': data['datetime'].strftime('%Y-%m-%d %H:%M:%S')
            }
...
def execute_soap_request():
    ...
    try:
        ...
        soap_request, start_date = create_soap_request()
            ...
            <table class="status-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><strong>{{ device.plate }}</strong></td>
                        <td>{{ device.last_report }}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
...
@app.route('/')
def index():
    # Calcular el límite de tiempo (start_date) con timezone UTC
    end_date = datetime.now(timezone.utc)
    start_date = end_date - timedelta(minutes=15)
    
    # Preparar lista de dispositivos con su estado
    device_list = []
    for plate, status in device_status.items():
        is_ok = status['last_report'] >= start_date
        device_list.append({
            'plate': plate,
            'last_report': status['last_report_str'],
            'is_ok': is_ok
        })
    
    # Ordenar por dominio
    device_list.sort(key=lambda x: x['plate'])
